...
6. **Ask deep probing questions**:
   ...
   **Example question with recommendations (software):** `Conflicts` is `[S]` — solution architecture; always asked even when a frame defined the problem.
   ...
   Ask the user: "How should the system handle conflicts when two users edit simultaneously?" with header "Conflicts" and options:
   ...
   **Example question with recommendations (non-software — content/strategy):** `Depth` is `[D]` — diagnostic about audience/scope; skip if a frame brief already settled who this is for.
   ...
   Ask the user: "What depth of technical detail should the course module target?" with header "Depth" and options:
...
6. **Present findings and design options**:
   ...
   **Current State:**
   ...
   Then, if there are multiple valid approaches, present them as structured choices by asking the user:
   - "Which implementation approach should we use?" with header "Approach" and options:
...
1. **Present plan outline and get structured feedback**:
   ...
   Then ask the user: "Does this phase breakdown look right?" with header "Phases" and options:
...
Ask the user:
- "What should the user see while new items load?" with header "Loading UX" and options:
...
- "How many items should this handle gracefully?" with header "Scale" and options:
...
Ask the user:
- "What should the learner be able to DO after this module — not just know?" with header "Outcome" and options:
...
- "How should this module handle different skill levels in the audience?" with header "Levels" and options:
...
Ask the user:
- "What's the primary bottleneck in the current newsletter pipeline?" with header "Bottleneck" and options:
