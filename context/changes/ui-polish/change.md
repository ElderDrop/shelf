...
Visual system (revised): light brown / beige / white / black — parchment page (`background`), white surfaces (`card`), near-black type (`foreground`), walnut brown accent (`primary`). Product + auth surfaces use semantic tokens + `shelf-page` / `shelf-panel`. Home/dashboard still cosmic until Phase 4.
